...
class RDatasetFiveFoldOneModelityClass(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = torch.tensor(data['label_2'][0]*1.0, dtype=torch.float32)
        # print(labels)
        # sys.exit()
        # seg_labels = torch.tensor(data['seg_label'] / 255.0, dtype=torch.float32)
        clinical_inf = torch.tensor(data['clinical'], dtype=torch.float32)
        # return {'inputs': inputs, 'labels': labels, 'seg_labels': seg_labels}
        return {'inputs': inputs, 'labels': labels, 'clinical': clinical_inf}
