...
if __name__ == '__main__':
    csv_dir = r"../../../datasets/lex.csv"  # 按该路径下csv文件加二分类标签
    data_path = r"D:\Dataset\key_frame_10_all_mat_5_c\data"  # 源数据路径，有临床信息
    t_data_path = r"D:\Dataset\key_frame_10_all_mat_5_c_2_lex\data"  # 目标数据保存路径
    ...
    for i in range(len(csv_data) - 1):
        ...
        labels_2 = [0, 1]
        if csv_data[j][7] == '0':  # 良性
            labels_2 = [0, 1]
        elif csv_data[j][7] == '1':  # 恶性
            labels_2 = [1, 0]
        else:
        ...
        if labels_2[0] == 1:
            t_filename = csv_data[j][0] + "_1.mat"  # "良性" [01]
            t_mat_dir = os.path.join(t_data_path, t_filename)
        else:
            t_filename = csv_data[j][0] + "_0.mat"  # "恶性"  [10]
            t_mat_dir = os.path.join(t_data_path, t_filename)
        print(mat_dir)
        print(t_mat_dir)
