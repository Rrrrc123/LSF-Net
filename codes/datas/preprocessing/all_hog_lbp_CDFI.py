...
def adaptive_key_frame_selection(frames_list, threshold_max=1, threshold_mean=1, sgm=1.25,
    ...
    if len(frames_list) > KEY_FRAMES_NUM:
        ...
        for i in range(len(frames_list) - 1):
            ...
            threshold = normal_fun(x[j], sgm, 0) / max_v * threshold_max * threshold_mean + bias  # 0.92 最大相似度
...
def key_frame_extern(m_image_list_path, m_image_save_path):
    ...
    for i in range(len(m_file_list)):
        t_image = cv2.imread(m_file_list[i])
        # m_gray_image = cv2.cvtColor(m_image, cv2.COLOR_BGR2GRAY)
        m_image = copy.deepcopy(t_image[95:599, 156:696, :])
        r_image = m_image[:, :, 0]
        g_image = m_image[:, :, 1]
        b_image = m_image[:, :, 2]
        t1 = r_image - g_image
        ...
        t3[t3 > 0] = 1
        mask = (t1 + t2 + t3) / 3
        r_image = r_image * mask
        g_image = g_image * mask
        b_image = b_image * mask
        m_image[:, :, 0] = r_image
        m_image[:, :, 1] = g_image
        m_image[:, :, 2] = b_image
        m_image = cv2.cvtColor(m_image, cv2.COLOR_BGR2GRAY)
        m_frames_list.append(m_image)  #
    print("加载所有帧图像完成.......")
    ...
    while m_frames_idx[-1] / m_file_list_len < 0.8 or len(m_key_frames) < KEY_FRAMES_NUM:  # 15张图像
        m_key_frames, m_frames_idx, is_ok = adaptive_key_frame_selection(m_frames_list, m_correlation_list_max,
                                                                         ...
                                                                         bias=m_bias)
        print("m_bias：", m_bias, "提取帧数：", len(m_key_frames), "帧百分比：", m_frames_idx[-1] / m_file_list_len)
        if is_ok == 1:
            break
        ...
        if m_frames_idx[-1] / m_file_list_len < 0.8 and len(m_key_frames) > KEY_FRAMES_NUM:
            ...
            m_bias += 0.01
        j_count += 1
        if j_count > 200 and len(m_key_frames) >= KEY_FRAMES_NUM:
            break
        elif j_count > 400:
            break
...
if __name__ == '__main__':
    if __name__ == '__main__':
        ...
        parser.add_argument('--source_dir', type=str,
                            default=r'D:\Dataset\key_frame_test\CDFI_IMG',
                            help='the directory which is used to store the extracted frames')
        parser.add_argument('--target_dir', type=str,
                            default=r'D:\Dataset\key_frame_test\CDFI_KEY',
                            help='the directory which is used to store the extracted frames')
