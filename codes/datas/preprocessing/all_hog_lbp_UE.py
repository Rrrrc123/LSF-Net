...
def key_frame_extern(m_image_list_path, m_image_save_path):
    ...
    print("开始提取关键帧.....")
    j_count = 0
    while m_frames_idx[-1] / m_file_list_len < 0.8 or len(m_key_frames) < KEY_FRAMES_NUM:  # 15张图像
        ...
        if m_frames_idx[-1] / m_file_list_len > 0.8 and len(m_key_frames) > KEY_FRAMES_NUM:
            break
        ...
        if m_frames_idx[-1] / m_file_list_len < 0.8 and len(m_key_frames) > KEY_FRAMES_NUM:
            ...
            m_bias += .003
        j_count += 1
        if j_count > 500 and len(m_frames_idx) > KEY_FRAMES_NUM and m_frames_idx[-1] / m_file_list_len > 0.6:
    ...
    for i in range(len(m_key_frames)):
        ...
        print(key_frame_file_name, m_frames_idx[i])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_dir', type=str,
                        default=r'D:\Dataset\key_frame_test\UE_IMG',
                        help='the directory which is used to store the extracted frames')
    parser.add_argument('--target_dir', type=str,
                        default=r'D:\Dataset\key_frame_test\UE_KEY',
                        help='the directory which is used to store the extracted frames')
