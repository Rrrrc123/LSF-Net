# coding:utf-8
# File:       all_hog_lbp_UE_ROI.py
# Author:     tfsy-rifnyga
# Created on: 2024-11-08
# function: 根据ROI区域提取UE关键帧
...
def calculate_pearson_correlation_hl(frame1, frame2):
    ...
    gray_image_2 = cv2.cvtColor(frame2, cv2.COLOR_BGR2GRAY)
    gray1_hog = get_hog_vector(gray_image_1)
    gray2_hog = get_hog_vector(gray_image_2)
    gray1_lbp = get_lbp_vector(gray_image_1)
    gray2_lbp = get_lbp_vector(gray_image_2)
    # 将图像展平为一维数组
...
# def normal_fun(x_v, f, bias):
#     return math.fabs(0.3 * math.cos(2 * math.pi * x_v * f + bias))
...
def adaptive_key_frame_selection(frames_list, threshold_max=1, threshold_mean=1, sgm=1.25,
    ...
    for i in range(len(frames_list)):
        ...
        r_sum = current_frame[:, :, 0].sum()
        g_sum = current_frame[:, :, 1].sum()
        b_sum = current_frame[:, :, 2].sum()
        m_frames_means.append(math.fabs(r_sum - b_sum) / g_sum)
    frames_means = sum(m_frames_means) / len(m_frames_means)
    # print(len(m_frames_means), max(m_frames_means), min(m_frames_means), sum(m_frames_means)/len(m_frames_means))
    # key_frames = []  # 将第一帧作为关键帧
    # frames_idx = []
    # return key_frames, frames_idx, 0
    # sys.exit()

    if len(frames_list) < KEY_FRAMES_NUM:
        for i in range(len(frames_list)):
            m_frames_list.append(frames_list[i])
            m_frames_idx.append(i)
        return m_frames_list, m_frames_idx, len(m_frames_idx)

    for i in range(len(frames_list)):
        if m_frames_means[i] < frames_means:
            m_frames_list.append(frames_list[i])
            m_frames_idx.append(i)
    # 初始化关键帧列表//
    if len(m_frames_list) <= KEY_FRAMES_NUM:
        return m_frames_list, m_frames_idx, len(m_frames_idx)
    key_frames = []  # 将第一帧作为关键帧
    frames_idx = []
    x = np.fabs(np.cos(math.pi * np.linspace(0, 200, 199) / 2.0))*0.2+0.8# 正态分布的X取值
    # print(x)
    j = 0
    max_v = normal_fun(0.0, sgm, 0)
    # max_v = 1
    key_1 = 0
    prev_frame = m_frames_list[0]
    frames_idx.append(m_frames_idx[0])

    for i in range(len(m_frames_list) - 1):
            # 计算当前帧与前一关键帧之间的皮尔逊相关系数
            current_frame = m_frames_list[i + 1]
            correlation = calculate_pearson_correlation(prev_frame, current_frame)

            threshold = normal_fun(x[j], sgm, 0) / max_v + bias  # 0.92 最大相似度
            if correlation < threshold:
                key_frames.append(current_frame)
                frames_idx.append(m_frames_idx[i+1])
                prev_frame = current_frame
                j += 1
            if j > KEY_FRAMES_NUM + 1:
                break
    return key_frames, frames_idx, len(m_frames_idx)
...
def key_frame_extern(m_image_list_path, m_mask_list_path, m_image_save_path):
    ...
    for file_name in m_file_list:
        ...
        m_gray_image = m_image * m_mask
        # m_gray_image = m_image[199: 439, 457: 762, :]
    ...
    m_bias = m_correlation_list_min
    print("开始提取关键帧.....")
    j_count = 0
    m_key_frames, m_frames_idx, len_frames_idx = adaptive_key_frame_selection(m_frames_list, m_correlation_list_max,
                                                                              m_correlation_list_mean, sgm=m_sgm,
                                                                              bias=m_bias)
    while m_frames_idx[-1] / m_file_list_len < 0.4 or len(m_key_frames) < KEY_FRAMES_NUM:  # 15张图像
        m_key_frames, m_frames_idx, len_frames_idx = adaptive_key_frame_selection(m_frames_list, m_correlation_list_max,
                                                                  m_correlation_list_mean, sgm=m_sgm, bias=m_bias)
        if len_frames_idx == 0:
            return
        print(j_count, "m_bias：", m_bias, "提取帧数：", len(m_key_frames), "帧百分比：", m_frames_idx[-1] / m_file_list_len)
        if m_frames_idx[-1] / m_file_list_len > 0.40 and len(m_key_frames) > KEY_FRAMES_NUM:
            break
        elif len(m_frames_idx)/len_frames_idx > 0.9:
            break
        if m_frames_idx[-1] / m_file_list_len < 0.40 and len(m_key_frames) > KEY_FRAMES_NUM:
            m_bias -= .01
        else:
            m_bias += .003
        j_count += 1
        if j_count > 100 and len(m_frames_idx) > KEY_FRAMES_NUM and m_frames_idx[-1] / m_file_list_len > 0.6:
            print("waring:", m_image_list_path, m_frames_idx)
            break
        if j_count > 200:
            print("waring:", m_image_list_path, m_frames_idx)
            break
    ...
    for i in range(len(m_key_frames)):
        ...
        raw_json_file_name = str.replace(raw_json_file_name, 'raw_frame_ue', 'raw_frame_ue_new')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_dir', type=str,
                        default=r'G:\key_frame_processing\raw_frame_ue\ex1\ue',
                        help='the directory which is used to store the extracted frames')
    parser.add_argument('--mask_dir', type=str,
                        default=r'G:\key_frame_processing\raw_frame_ue_mask\ex1\ue',
                        help='the directory which is used to store the extracted frames')
    parser.add_argument('--target_dir', type=str,
                        default=r'G:\key_frame_processing\raw_frame_ue_key\ex1\ue',
                        help='the directory which is used to store the extracted frames')
