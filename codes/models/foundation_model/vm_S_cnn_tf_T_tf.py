...
class TimeBlocker(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=input_size, nhead=8, dim_feedforward=128, dropout=0.1,
                                                   batch_first=True, norm_first=False)
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=3, enable_nested_tensor=True)
...
class ShareToIndependentMlp(nn.Module):
    def __init__(self):
        super(ShareToIndependentMlp, self).__init__()
        self.fc1 = nn.Linear(VECTOR_LEN * 2, VECTOR_LEN * 1)
        self.dp1 = nn.Dropout(0.2)
        self.fc2 = nn.Linear(VECTOR_LEN * 1, VECTOR_LEN * 2)
...
class ShareExpertBlocker(nn.Module):
    def __init__(self):
        super(ShareExpertBlocker, self).__init__()
...
class ShareExpert(nn.Module):
    def __init__(self):
        ...
        self.share_expert = ShareExpertBlocker()

    def forward(self, x, mask_code):
        x = self.share_expert(x)
        return x
...
class MulModalityMaeMoe(nn.Module):
    def __init__(self):
        ...
        self.s_to_i_list = nn.ModuleList([ShareToIndependentMlp() for _ in range(4)])
    ...
    def forward(self, x, mask_code):
        ...
        for i in range(4):
            if mask_code[i] == 1:
                ...
                encoded_s_to_i = self.s_to_i_list[i](encoded_s)

                encoded_i = self.independentExpert_list[i](encoded)

                encoded_i_list[:, i * 10:(i + 1) * 10, :] = encoded_i + encoded_s_to_i
