...
import sys
from codes.datas.classification_2_m.r_dataset_load_mat_2 import *
from codes.models.classification_2_m.vm_classification_2_m_model import *
...
class OptimizerParamClass:
    def __init__(self):
        ...
        self.prior_knowledge = torch.tensor([1, 0.5, 0.2, 0.18, 0.04])

        self.datasets_path = r"D:\Dataset\key_frame_10_all_mat_5_c_2_ex_zj_a"
        self.models_save_path = "../../../checkpoints/classification_2_m/" + self.model_name  # 模型保存路径
        self.results_save_path = "../../../results/classification_2_m/" + self.model_name  # 模型保存路径
        self.wandb_project = ""
...
if __name__ == '__main__':
    ...
    mpe = MulModalBinClassificationEvaluation()
    ...
    for fold_num in range(len(opt.fold_num_list)):
        ...
        model = Classification2MModel(opt.package_name, opt.model_name, opt.class_name, opt.prior_knowledge)
        ...
        test_log_csv.set_table_head(
            ["epoch", "loss", "t_acc", "t_f1_score", "t_sen(recall)", "t_spe", "t_auc", "t_precision"])
        train_log_csv.set_table_head(
            ["epoch", "loss", "t_acc", "t_f1_score", "t_sen(recall)", "t_spe", "t_auc", "t_precision"])
        ...
        for epoch in range(num_epochs):
            ...
            if opt.is_train_log == 1:
                print("train_result:")
                log_row = mpe.get_models_performance_evaluation(model, datasets)
            ...
            print("test_result:")
            log_row = mpe.get_models_performance_evaluation(model, test_datasets)
            # mpe.print_specificity_list()
            # mpe.print_sensitivity_list()
            # mpe.print_confusion_matrix()
